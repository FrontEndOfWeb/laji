...
(function (win, doc) {
  ...
  function setRemUnit() {
    ...
    styleN = doc.createElement('style');
    rem = (docWidth / 750) * 100;
    styleN.innerHTML = 'html{font-size:' + rem + 'px !important;}body{font-size: 14px}'
    doc.head.appendChild(styleN);
  }

  setRemUnit()
  win.addEventListener('resize', function () {
    clearTimeout(tid);
    tid = setTimeout(setRemUnit, 100)
  }, false);
  ...
  win.addEventListener('load', setRemUnit, false);
})(window, document)
...
(function ($) {
  //app传的url 是这样子的  (有两个？)
  // http://10.0.253.157:35092/productionTask/getDetail?enterpriseId=55&sessionID=5125a83030c94ee4be400db7fca4687b&id=1028967793244176384%3FsessionId%3D0f88aa8bdb544bb0abb2e5939dc033c1&actionCode=
  $.getUrlParam = function (name) {
    var url = decodeURIComponent(window.location.href),
        splitUrl, obj = {}, str = '';
    if (url.indexOf('?') != -1) {
      splitUrl = url.split('?');
      $.each(splitUrl, function (index1, item1) {
        var strArr = item1.split('&');
        $.each(strArr, function (index2, item2) {
          if (item2.split('=')[1]) {
            obj[item2.split('=')[0]] = item2.split('=')[1]
          }
        })
      })
    }
...
function FastClick(a) {
  ...
  this.layer = a;
  FastClick.notNeeded(a) || (deviceIsAndroid && (a.addEventListener("mouseover", b(this.onMouse, this), !0), a.addEventListener(
      "mousedown", b(this.onMouse, this), !0), a.addEventListener("mouseup", b(this.onMouse, this), !0)), a.addEventListener(
      "click", b(this.onClick,
          this), !0), a.addEventListener("touchstart", b(this.onTouchStart, this), !1), a.addEventListener("touchmove", b(
      this.onTouchMove, this), !1), a.addEventListener("touchend", b(this.onTouchEnd, this), !1), a.addEventListener(
      "touchcancel", b(this.onTouchCancel, this), !1), Event.prototype.stopImmediatePropagation || (a.removeEventListener = function (b, c, e) {
...
FastClick.prototype.sendClick = function (a, b) {
  ...
  c.initMouseEvent(this.determineEventType(a), !0, !0, window, 1, d.screenX, d.screenY, d.clientX, d.clientY, !1, !1, !
      1, !1, 0, null);
...
FastClick.prototype.focus = function (a) {
  var b;
  deviceIsIOS && a.setSelectionRange && 0 !== a.type.indexOf("date") && "time" !== a.type ? (b = a.value.length, a.setSelectionRange(
      b, b)) : a.focus()
};
...
FastClick.prototype.onTouchMove = function (a) {
  ...
  if (this.targetElement !== this.getTargetElementFromEventTarget(a.target) || this.touchHasMoved(a)) this.trackingClick = !
      1, this.targetElement = null;
  return !0
};
...
FastClick.prototype.onTouchEnd = function (a) {
  ...
  this.trackingClick = !1;
  this.trackingClickStart = 0;
  deviceIsIOSWithBadTarget && (c = a.changedTouches[0], d = document.elementFromPoint(c.pageX - window.pageXOffset, c
      .pageY - window.pageYOffset) || d, d.fastClickScrollParent = this.targetElement.fastClickScrollParent);
  ...
  if ("label" === c) {
    ...
    if (100 < a.timeStamp - b || deviceIsIOS && window.top !== window && "input" === c) return this.targetElement =
        null, !1;
    this.focus(d);
    this.sendClick(d, a);
...
FastClick.prototype.onMouse = function (a) {
  return this.targetElement && !a.forwardedTouchEvent && a.cancelable ? !this.needsClick(this.targetElement) || this.cancelNextClick ?
      (a.stopImmediatePropagation ? a.stopImmediatePropagation() : a.propagationStopped = !0, a.stopPropagation(), a.preventDefault(), !
          1) : !0 : !0
};
...
FastClick.prototype.destroy = function () {
  var a = this.layer;
  deviceIsAndroid && (a.removeEventListener("mouseover", this.onMouse, !0), a.removeEventListener("mousedown", this.onMouse, !
      0), a.removeEventListener("mouseup", this.onMouse, !0));
...
FastClick.notNeeded = function (a) {
  ...
  if (c = +(/Chrome\/([0-9]+)/.exec(navigator.userAgent) || [, 0])[1]) if (deviceIsAndroid) {
    if ((b = document.querySelector("meta[name=viewport]")) && (-1 !== b.content.indexOf("user-scalable=no") ||
            31 < c && window.innerWidth <= window.screen.width)) return !0
  } else return !0;
...
"undefined" !== typeof define && define.amd ? define(function () {
  return FastClick
    ...
    FastClick) : window.FastClick = FastClick;
...
(function () {
  ...
  /**添加window对象hScoll属性*/
  window.hScoll = {
    buildScoll: function (el, options, contentEl) {
      App.init(el, options, contentEl);
    }
  }
  ...
  var App = {
    ...
    /**初始化组件*/
    init: function (el, option, contentEl) {
      App.options = option;
      App.prevY = 0;
      App.el = document.getElementById(el);
      // App.scoll = this.el.children[0];
      App.scoll = document.getElementById(contentEl);
    ...
    domove2: function (y, t) {
      if (t) {
        App.scollb.setAttribute('style', 'transform: translate(0px, ' + -parseFloat(y) * parseFloat(App.h) / parseFloat(App.ch) + 'px);transition:background 300ms ease;height:' + parseFloat(App.h) * parseFloat(App.h) / parseFloat(App.ch) + 'px' + ';background:#fff');
      } else {
        App.scollb.setAttribute('style', 'transform: translate(0px, ' + -parseFloat(y) * parseFloat(App.h) / parseFloat(App.ch) + 'px);transition:background 300ms ease;height:' + parseFloat(App.h) * parseFloat(App.h) / parseFloat(App.ch) + 'px' + ';background:#fff');
      }
    }
...
$(function () {
  //    后退
  $('.rps_header i').on('click', function () {
    history.back()
  });
  ...
  $(window).resize(function () {
    if ($(document).height() < oHeight) {
      $(".rps_bottom").css("visibility", "hidden");
    } else {
      $(".rps_bottom").css("visibility", "visible");
    }
  });
})
...
;(function () {
  var rpsLayer = {
    ...
    open: function (option) {
      ...
      var wrapperEl = $(['<div class="rps_layer_area">',
        '<div class="rps_layer" id="rps_layer">',
        '<div class="rps_layer_header">',
        '<span class="rps_title">',
        layerInfo.title,
        '</span>',
        ...
        '<input type="button" data-type="layer-yes" class="rps_btn" value="', layerInfo.btns[0], '">',
        '<input type="button" data-type="layer-cancel" class="rps_btn" value="', layerInfo.btns[1], '">',
        '</div>',
        '</div>',
        '</div>'].join(''));
      ...
      //页面打开后的回调
      if (layerInfo.success && typeof (layerInfo.success) === 'function') {
        layerInfo.success();
      }
      //按钮事件
      $(".rps_layer").on('click', '.rps_btn', function () {
        // console.log($(this).data().type);
        var type = $(this).data().type, btnType = ['layer-yes', 'layer-cancel'];
        if (type === btnType[0] && option.yes) {
          //yes
          option.yes()
        } else if (type === btnType[1]) {
          //cancel
          if (!self.canClose) {
            alert('数据加载中...')
          } else {
      ...
      //右上角的关闭按钮
      $(".rps_layer_delete i").on('click', function () {
    ...
    afterAjax: function () {
      ...
      window.hScoll.buildScoll('rps_layer', options1, 'rps_layer_content');
      this.canClose = true;
      console.log(this.canClose)
    }
  };
  window.rpsLayer = window.rpsLayer || rpsLayer;
})();

function getCookies() {
  var defaultArr = ["sessionId"],
      arr = arguments.length ? arguments : defaultArr,
      len = arr.length,
      cookies = {};
  for (var i = 0; i < len; i++) {
    cookies[arr[i]] = Cookies.get(arr[i])
  }
  //  将sessionId转换为sessionID
  ...
  //  部署时不要下边三行代码
  cookies = {
    sessionID: '0033fe50e8564534953b20574e430215',
  };
...
var rpsGlobalObj = window.rpsGlobalObj || {};

var server = false;
if (server) {
  rpsGlobalObj.protocol = window.location.protocol;
  rpsGlobalObj.host = window.location.host;
  rpsGlobalObj.urlArr = [rpsGlobalObj.protocol, '//', rpsGlobalObj.host, '/'];
  rpsGlobalObj.RPS_URL = rpsGlobalObj.urlArr.join('');

  // rpsGlobalObj.enterpriseId = $.getUrlParam('enterpriseId');
  rpsGlobalObj.sessionID = $.getUrlParam('sessionID');
  rpsGlobalObj.currentAssignee = $.getUrlParam('currentAssignee');

  //任务id
  rpsGlobalObj.id = $.getUrlParam('id');

  //预算id
  // rpsGlobalObj.budgetId = $.getUrlParam('budgetId');
} else {
  rpsGlobalObj.RPS_URL = 'http://10.0.253.157:35092/';

  rpsGlobalObj.enterpriseId = '55';
  rpsGlobalObj.sessionID = '6b5607882d7a4cf3998d213b4364e4ec';
  rpsGlobalObj.currentAssignee = '不惑';
  rpsGlobalObj.id = '1029689088252489729';
  // rpsGlobalObj.budgetId = '1028166082711724032';
}